...
def feedback_linearized(pose, velocity, epsilon):
  ...
  if np.linalg.norm(velocity) < 1e-2:
    return u, w
  # Get difference between desired YAW and current YAW
  d_rot_vel = np.arctan2(velocity[Y], velocity[X]) - pose[YAW]
  # Deal with -np.pi = np.pi angles
  d_rot_vel = np.mod(d_rot_vel + np.pi, 2*np.pi) - np.pi
  ...
  # Allow robot to reverse -> replace -MAX_SPEED with 0 to disable this
  u = np.clip(d_x_p*np.cos(pose[YAW]) + d_y_p*np.sin(pose[YAW]), -MAX_SPEED, MAX_SPEED)
  w = (-d_x_p*np.sin(pose[YAW]) + d_y_p*np.cos(pose[YAW])) / epsilon
  return u, w


def get_relative_position(absolute_pose, absolute_position):
  relative_position = absolute_position.copy()
  # Translate to absolute_pose frame
  relative_position = relative_position - absolute_pose[:2]
  # Rotate to absolute_pose frame
  s, c = np.sin(-absolute_pose[YAW]), np.cos(-absolute_pose[YAW])
  rot_mat = np.array(((c, -s), (s, c)))
  relative_position = np.matmul(rot_mat, relative_position)
