...
def get_velocity(position, path_points, speed):
  ...
  if np.linalg.norm(position - path_points[-1]) < .15 - EPSILON:
    return v
  
  # Find closest point on the path to me
  closest_point_on_path = np.argmin(np.linalg.norm(np.subtract(path_points, position), axis=1))
  # Get vector to the points after that
  average_points = 4
  for i in range(1, average_points + 1):
...
def get_path(final_node):
  ...
  positions = [p.pose[:2] for p in path]
